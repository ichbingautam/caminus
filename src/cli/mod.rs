...
impl CliHandler {
        ...
        T: Into<String>,
    {
        ...
        match vec[1].as_str() {
            ...
            "inspect-offset" => {
                let source_id = vec.get(2).cloned().unwrap_or_else(|| "postgres_users".to_string());
    ...
    pub fn execute(command: CliCommand, store: &StateStore) -> Result<String, String> {
        match command {
            CliCommand::Status => {
                Ok("Caminus CDC Engine Status: HEALTHY | Active Consensus: Node 1 (LEADER)".to_string())
            }
            CliCommand::InspectOffset { source_id } => {
                match store.get_offset(&source_id) {
                    Ok(Some(offset)) => Ok(format!("Source '{}' offset checkpoint: {}", source_id, offset)),
                    Ok(None) => Ok(format!("Source '{}' offset checkpoint: NONE", source_id)),
                    Err(e) => Err(format!("Storage error: {:?}", e)),
                }
            }
            CliCommand::SchemaList { source_id } => {
                match store.get_schema(&source_id) {
                    Ok(Some(schema)) => Ok(format!("Registered schema for '{}': {}", source_id, schema)),
...
mod tests {
    ...
    fn test_cli_parsing_and_execution() {
        ...
        assert_eq!(cmd, CliCommand::InspectOffset { source_id: "pg_users".to_string() });
